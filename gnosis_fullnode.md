...
```
## 3. Execution için Nethermind kullanacağız.
```
screen -S gnosis
```
```
mkdir execution && wget https://github.com/NethermindEth/nethermind/releases/download/1.14.6/nethermind-linux-amd64-1.14.6-6aea772-20221121.zip
unzip nethermind-linux-amd64-1.14.6-6aea772-20221121.zip -d execution
cd execution
```
## 4. Nethermind başlatalım.
```
./Nethermind.Runner \
--config xdai \
--Sync.FastSync true \
...
```
## 5. Consensus için Lighthouse kullanacağız.
...
cd consensus
```
## 6. Lighthouse başlatalım.
```
./lighthouse \
--network gnosis beacon_node \
--http \
--execution-endpoint http://localhost:8551 \
--execution-jwt /root/execution/~/jwtsecret/jwt.hex \
